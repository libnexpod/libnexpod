...
pub fn listContainers(allocator: std.mem.Allocator, key: []const u8) ![]Container {
    ...
    var result = try std.ArrayListUnmanaged(Container).initCapacity(allocator, amount);
    errdefer {
        for (result.items) |e| {
            e.deinit();
        }
    ...
    while (iter.next()) |next| {
        result.appendAssumeCapacity(try getContainer(allocator, key, next));
    }

    return try result.toOwnedSlice(allocator);
}

pub fn getContainer(allocator: std.mem.Allocator, key: []const u8, id: []const u8) !Container {
    if (utils.isInsideContainer() and !utils.isInsideLibnexpodContainer()) {
        return errors.LibnexpodErrors.InsideNonLibnexpodContainer;
    }
    ...
    const con = try parseContainer(allocator, json);
    errdefer con.deinit();
...
fn parseContainer(allocator: std.mem.Allocator, json: []const u8) !Container {
    var tmp_arena = std.heap.ArenaAllocator.init(allocator);
    defer tmp_arena.deinit();
    const tmp_allocator = tmp_arena.allocator();
