...
pub const ListErrors = LibnexpodErrors || PodmanErrors || std.json.ParseFromValueError || std.json.Scanner.AllocError || std.json.Scanner.NextError || std.process.Child.RunError || std.Io.Writer.Error;

/// all images which can happen when update the info of a container
pub const UpdateErrors = std.json.ParseError(std.json.Scanner) || std.process.Child.RunError || PodmanErrors || std.mem.Allocator.Error || std.Io.Writer.Error;
